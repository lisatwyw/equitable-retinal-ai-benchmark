...
#   BASELINE:
#       - one backbone
#       - L and R are independent training examples
#       - sees exactly the same eye images as the proposed student
#
#   PROPOSED:
#       - teacher sees paired L + R
#       - teacher learns a bilateral privileged representation
#       - student sees L and R independently
#       - student learns classification + distillation from teacher
#       - no LoRA required
#
#   RES1=RES2=70*14; TK='A'; exec( open('compare.py').read() ) 
#
# ============================================================
...
def split_dataset_by_patient(df, args ):
    ...
    #train_val_idx, test_idx = next(gkf.split(df, groups=df["patient_id"]))
...
class CFPDataset(Dataset):
    ...
    def __init__(self, dataframe, target_cols, image_dir, CENTROID_CROP=True, target_size=(392, 392), image_col_left="Left-Fundus", image_col_right="Right-Fundus", single_image=False, file_format=None):
        self.CENTROID_CROP = conf.CENTROID_CROP
...
# max-pooling is permutation-invariant and does not require
# learning an additional fusion network.
# ============================================================
...
def make_pos_weight(df, task):
    n_positive = float(df[task].values.sum())
    n_negative = float(len(df) - n_positive)
    return torch.tensor([ np.log( n_negative / max(n_positive, 1.0) )**2 ], device=DEVICE)
...
def train_privileged(model, train_loader, val_loader, trn_df, conf):
    ...
    for epoch in range(1, conf.MAX_EPOCHS + 1):
        ...
        for left, right, targets, _ in train_loader:
            ...
            z_teacher_student = torch.cat(
                [z_teacher, z_teacher],
                dim=0
            )

            loss_student_cls = criterion(
                student_logits,
                y_student
            )
        ...
        val_loss, val_auc = evaluate_validation_single_eye(
            model.student,
            val_loader,
            criterion
        )
...
def train_privileged_logit_kd(model, train_loader, val_loader, trn_df, conf):
    """Trains a single-eye student via Temperature Logit Distillation from a frozen

    privileged bilateral teacher.

    Key Benefits over Feature MSE:
    ...
    for epoch in range(1, conf.MAX_EPOCHS + 1):
        ...
        for left, right, targets, _ in train_loader:
            ...
            with torch.no_grad():
                ...
                # Duplicate privileged teacher predictions for both L and R student inputs
                teacher_logits_rep = torch.cat(
                    [teacher_logits, teacher_logits], dim=0
                )
            ...
            # Soften probabilities with T > 1, then scale gradient by T^2
            # -----------------------------------------------------------------
            soft_teacher_probs = torch.sigmoid(teacher_logits_rep / T)
            soft_student_logits = student_logits / T

            loss_soft = (T**2) * F.binary_cross_entropy_with_logits(
                soft_student_logits, soft_teacher_probs
            )
        ...
        val_loss, val_auc = evaluate_validation_single_eye(
            model.student, val_loader, criterion_hard
        )
...
def get_predictions_df(model, loader, eye, threshold, dataset_name="ODIR"):
    """
    Returns a Pandas DataFrame with predictions and metadata.
    `eye` can be "left", "right", or "single".
    """
    y_true, y_prob, indices = get_single_eye_predictions(model, loader, eye)
    y_pred = (y_prob >= threshold).astype(int)

    # Get the underlying dataset and its samples DataFrame
    dataset = loader.dataset
    df_samples = dataset.samples.iloc[indices].reset_index(drop=True)

    # Build the output DataFrame
    out_df = pd.DataFrame({
        "patient_id": df_samples.get("patient_id", df_samples.index),  # fallback to index
        "y_true": y_true,
        "y_prob": y_prob,
        "y_pred": y_pred,
        "threshold": threshold,
    ...
    # If image filenames are available, add them
    ...
    elif "image_id" in df_samples.columns:  # For BRSet
...
def evaluate_test(model, loader, eye, threshold, n_bootstraps=1000, alpha=0.05):
    ...
    """
    y_true, y_prob = get_single_eye_predictions(model, loader, eye)
    ...
    for _ in range(n_bootstraps):
        ...
        # Avoid crashes if bootstrap sample has only one class
        ...
        boot_records["Balanced_Accuracy"].append(balanced_accuracy_score(y_t, y_pred_b))
        boot_records["F1"].append(f1_score(y_t, y_pred_b, zero_division=0))
        boot_records["F1_Macro"].append(f1_score(y_t, y_pred_b, average='macro', zero_division=0))
        ...
        for batch in loader:
            if len(batch) == 3:  
                ...
                try: # Try treating it as a single-eye model
                    ...
                    x = x.to(DEVICE, non_blocking=True)
                    logits, feats = model(x)                                   
                except Exception as e:   # Fallback: Treat as a dual-eye model if single-eye fail
            ...
            ps.append( torch.sigmoid( logits[:, 0]).cpu().numpy() ) # signmoid function, so not a multi-column
            idxs.append(idx.cpu().numpy())    
            features.append( feats.cpu().numpy() )
    return np.concatenate(ys), np.concatenate(ps), np.concatenate(idxs) # np.concatenate(features)
...
'''
    df = pd.concat([ canon_df.assign(dataset="Canon"), secondary_df.assign(dataset="Nikon-abnormal"),  external_df.assign(dataset="Nikon-normal")], ignore_index=True)
...
if 0:
    ...
    #cfp_df['file_exists'] = cfp_df['image_path'].apply( lambda x: image_exists( x))      
    cfp_df.drop(columns="image_path",inplace=True,)            
'''
...
def define_brset_splits( df ):
    ...
    def split( cfp_df ):    
        ...
        # R = nikon.resolution.dropna().unique()
        # R = ['951x874', '990x874', '2984x2304', '2390x1880', '2672x2056', '2420x1880']
        ...
        overlap = set(nikon["patient_id"]) & set(canon_df["patient_id"]); print("Number of overlapping patient IDs:", len(overlap))    
        overlap = set(secondary_df["patient_id"]) & set(external_df["patient_id"]); print("Number of overlapping patient IDs:", len(overlap))    
...
''' # not used
def get_external_spits(n=2):
    gkf = GroupKFold(n_splits=n)
    train_val_idx, test_idx = next(gkf.split(ext_df, groups=ext_df["patient_id"]))
    ext_df_sub = ext_df.iloc[train_val_idx].reset_index(drop=True)
    ext_df_cal = ext_df.iloc[test_idx].reset_index(drop=True)
    print('Size of calibration split:',ext_df_cal.shape, 'Size of remaining test:', ext_df_sub.shape)
    
    brset_cal = CFPDataset( ext_df_cal, target_cols=conf.task, image_dir=CFP_IMAGE_DIR, image_col_left="image_id", image_col_right=None, single_image=True, file_format="jpg", CENTROID_CROP=conf.CENTROID_CROP )
    brset_sub = CFPDataset( ext_df_sub, target_cols=conf.task, image_dir=CFP_IMAGE_DIR, image_col_left="image_id", image_col_right=None, single_image=True, file_format="jpg", CENTROID_CROP=conf.CENTROID_CROP )  
    ext_loaders = {}
    ext_loaders[0] = DataLoader( brset_cal, batch_size=conf.BS, shuffle=True,  num_workers=ncores, pin_memory=pin_mem)    
    ext_loaders[1] = DataLoader( brset_sub, batch_size=conf.BS, shuffle=True,  num_workers=ncores, pin_memory=pin_mem)            
    return ext_df_cal, ext_df_sub, ext_loaders    
'''
...
if __name__ == "__main__":
    ...
    sides = ['left','right']
    MODES = ['baseline','privileged_student']
    MODES2 = ['baseline','privileged_student', 'teacher']
    ...
    with track_compute(f"Reading external dataset + generating figure of samples"):
        DSET='brset'
        #ext_df_cal, ext_df_sub, ext_loaders = get_external_spits(10)                            
    ...
    with track_compute(f"Reading DEV dataset + generating figure of samples"):
    ...
    try:       
        ...
        Models[MODE] = privileged_model.student # train_privileged returns the full wrapper; we only need the student for inference           
            ...
            for MODE in MODES2:            
                try:
                    ...
                    try:
                        ...
                        #get_predictions_df( preds[side,MODE,DSET,d][0], 
                        #preds[side,MODE,DSET,d][1], preds[side,MODE,DSET,d][2], thresholds[side,MODE], test_loader, side).to_csv( f"{pref}_{MODE}_preds.csv", index=False)
                    except:
                        pass            
...
#exec( open('cka.py').read() ) 
#exec( open('linear_prob.py').read() ) 
